...
dataset:2003 Stereo datasets with ground truth
"""
...
def compute_costs(left, right, parameters, save_images):
    ...
    if save_images:
        cv2.imwrite('output/left_census.png', left_img_census)
        cv2.imwrite('output/right_census.png', right_img_census)

    # left_census_values = cv2.imread('output/LBP_left.png', 0)
    # right_census_values = cv2.imread('output/LBP_right.png', 0)
...
def sgm():
    ...
    parser.add_argument('--left', default='../pic/left.jpg', help='name (path) to the left image')
    parser.add_argument('--right', default='../pic/right.jpg', help='name (path) to the right image')
    # parser.add_argument('--left_gt', default='ALL-2views/Baby1/disp1.png', help='name (path) to the left ground-truth image')
    # parser.add_argument('--right_gt', default='ALL-2views/Baby1/disp5.png', help='name (path) to the right ground-truth image')
    ...
    if save_images:
        cv2.imwrite('output/sgm/left_disp_map_no_post_processing.png', left_disparity_map)
        cv2.imwrite('output/sgm/right_disp_map_no_post_processing.png', right_disparity_map)
    ...
    # cv2.imwrite(f'output/left_{output_name}', left_disparity_map)
    # cv2.imwrite(f'output/right_{output_name}', right_disparity_map)
    cv2.imwrite('output_dir/sgm/'+'left_'+output_name, left_disparity_map)
    cv2.imwrite('output_dir/sgm/'+'right_'+output_name, right_disparity_map)

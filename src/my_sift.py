...
output_dir = '../output_dir/sift/my_output/'
...
if __name__ == "__main__":
    image = cv2.imread('../pic/left.jpg',0)
    image_rgb = cv2.imread('../pic/left.jpg')
    ...
    cv2.imwrite(output_dir + 'sift.jpg', np.asarray(key_points_detection[0][0]))
    print(key_points_detection[0][0].shape)
    for i in range(0, key_points_detection[0][0].shape[0]):
        for j in range(0, key_points_detection[0][0].shape[1]):
            if key_points_detection[0][0][i][j] != 0:
                pt = (j, i)
                cv2.circle(image_rgb, pt, 3, [0, 0, 255], 1, cv2.LINE_AA)

    cv2.imwrite(output_dir + 'sift_image.jpg', image_rgb)
